<template>
  <div class="p-3 sm:p-6 bg-white dark:bg-gray-900 rounded-xl dark:shadow-green-400 dark:border-none border shadow-lg border-green-200">
    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
      ...
      >
        <!-- Thumbnail -->
        <div class="h-20 sm:h-28 relative">
          <img
          ...
          />
          <div v-else class="w-full h-full bg-gradient-to-br from-green-500 to-emerald-600 flex items-center justify-center text-white">
            <Icon name="material-symbols:menu-book" class="w-8 h-8 sm:w-12 sm:h-12" />
          </div>
          <span
            v-if="material.status === 'draft'"
            class="absolute top-1.5 right-1.5 sm:top-2 sm:right-2 px-1.5 sm:px-2 py-0.5 text-[9px] sm:text-[10px] uppercase font-bold rounded-full bg-amber-100 text-amber-700"
          >
            Draft
          </span>
          <button
            @click.stop="deleteMateri(material.id)"
            class="absolute top-1.5 left-1.5 sm:top-2 sm:left-2 w-6 h-6 sm:w-7 sm:h-7 flex items-center justify-center rounded-full bg-red-500 text-white hover:bg-red-600 transition sm:opacity-0 sm:group-hover:opacity-100"
            title="Hapus materi"
          >
        ...
        <div class="p-2.5 sm:p-4 flex flex-col flex-1">
          <h3 class="font-semibold text-xs sm:text-base text-gray-800 dark:text-gray-100 group-hover:text-green-600 line-clamp-2">
            {{ material.title }}
          </h3>
          <p class="text-[11px] sm:text-sm text-gray-500 mt-0.5 sm:mt-1 line-clamp-2 leading-relaxed">{{ material.description }}</p>

          <!-- Meta badges -->
          <div class="mt-1.5 sm:mt-3 flex flex-wrap gap-1 sm:gap-1.5 text-[9px] sm:text-xs">
            <span v-if="material.subject" class="px-1 sm:px-2 py-0.5 rounded-full bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300">
              {{ material.subject }}
            </span>
            <span v-if="material.phase" class="px-1 sm:px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300">
              Fase {{ material.phase }}{{ material.class_level ? " · " + material.class_level : "" }}
            </span>
            <span
              v-if="material.category === 'interactive'"
              class="px-1 sm:px-2 py-0.5 rounded-full bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300"
            >
          ...
          <!-- Meta info -->
          <div class="mt-1.5 sm:mt-2 flex flex-wrap items-center gap-x-2 sm:gap-x-3 gap-y-0.5 text-[9px] sm:text-xs text-gray-400">
            <span v-if="material.topic" class="flex items-center gap-0.5 sm:gap-1">
              <Icon name="material-symbols:category" class="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5" />
              <span class="truncate max-w-[100px] sm:max-w-[140px]">{{ material.topic }}</span>
            </span>
            <span class="flex items-center gap-0.5 sm:gap-1">
              <Icon name="material-symbols:schedule" class="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5" />
          ...
          <!-- Action -->
          <div class="mt-2 sm:mt-3 pt-2 sm:pt-3 border-t border-gray-100 dark:border-gray-800">
            <NuxtLink
              v-if="material.category === 'interactive'"
              :to="`/teacher/materials/builder/${material.id}`"
              class="block w-full bg-green-600 hover:bg-green-700 text-white text-[11px] sm:text-sm px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg text-center transition font-medium"
            >
              <Icon name="material-symbols:auto-stories" class="w-3.5 h-3.5 sm:w-4 sm:h-4 inline mr-0.5 sm:mr-1" />
              Buka Materi
            </NuxtLink>
            <a
              v-else
              ...
              class="block w-full bg-green-600 hover:bg-green-700 text-white text-[11px] sm:text-sm px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg text-center transition font-medium"
            >
              <Icon name="material-symbols:open-in-new-rounded" class="w-3.5 h-3.5 sm:w-4 sm:h-4 inline mr-0.5 sm:mr-1" />
              Lihat Materi
            </a>
          </div>
        </div>
