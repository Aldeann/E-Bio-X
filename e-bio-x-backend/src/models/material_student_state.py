...
class MaterialStudentState(db.Model):
    ...
    last_content_id = db.Column(db.Integer, db.ForeignKey('material_contents.id'), nullable=True)
    total_learning_seconds = db.Column(db.Integer, nullable=False, default=0)
    ...
    last_section = db.relationship('MaterialSection', backref=db.backref('student_states', lazy=True, foreign_keys=[last_section_id]))
    last_content = db.relationship('MaterialContent', backref=db.backref('student_states', lazy=True, foreign_keys=[last_content_id]))
